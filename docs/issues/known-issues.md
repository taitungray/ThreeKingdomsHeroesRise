...
### 第二輪武將詳情稽核（2026-08-31）

依 390px 參考畫面重現並修正四維統計窄欄擠疊與 disabled 操作低對比：`.detail-hero .stat-list` 現為全寬雙欄單行，disabled 鐵灰按鈕不再使用暗化 filter。`npm run test:ui` 已取得並逐一開啟名冊全部 50 名武將，在零資源狀態檢查四個 stat pair、技能文案、44px 操作尺寸與 page error，結果通過；UI-005／UI-015 仍維持 `VERIFY`，因關閉條件還需要固定尺寸截圖與 Android／WebView／TalkBack 人工證據。
