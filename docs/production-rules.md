...
- UI、發布、目前遊戲規格與測試分別以 `ui-display-rules.md`、`platform-and-release.md`、`current-game-spec.md`、`qa-test-matrix.md` 為準。
...
以下任一項必須走 `qa-test-matrix.md` 對應流程：戰鬥狀態、波次、敵人生成、角色渲染、兵器、死亡、輸入、存檔、獎勵、背景恢復、廣告、原生橋接、build 或發布。
...
需要同步 `www/` 時執行 `node build.js`。只有使用者要求畫面測試時才啟動瀏覽器並依 `visual-qa.md` 截圖；正式發布另依平台文件走實機與商店流程。
