...
## 第二輪武將詳情對比修正與全角色驗證（2026-08-31）

本輪依 390px 武將詳情截圖逐項比對，確認兩個可重現問題：四維統計被限制在右側窄欄而產生中文字與數值擠疊；資源不足時的升級／升星／戰法按鈕被全域 disabled filter 壓暗，文字與底色對比不足。

已修正：四維統計改為詳情卡全寬雙欄 ledger，每個欄位保留單行標籤與數值；戰法說明改用深色紙卷文字；disabled 控制改用低飽和鐵灰表面與清晰淺色文字，維持至少 44px 觸控尺寸，不再以濾鏡降低文字可讀性。

自動化證據：`npm run test:ui` 會先取得名冊中的全部 50 名武將，再逐一開啟詳情；在金幣／糧草／玉璧／碎片歸零的 disabled 狀態下，檢查四個 stat pair 無 overflow、標籤為單行、控制尺寸與對比符合契約，並記錄無 page error。測試同時涵蓋 390×720、430×932、320×568 與 16 個面板；本證據仍不取代 Android／WebView／TalkBack 人工走查。
