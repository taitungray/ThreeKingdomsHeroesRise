...
| ID | 工作 | 對應問題 | 狀態 | 完成條件 |
|---|---|---|---|---|
| WORK-008 | 解耦面板與自動推關並重整命令面板表面 | UI-001、UI-002、UI-003、UI-005、UI-006、UI-007、UI-008、UI-009 | IN PROGRESS | `test:ui` 已斷言 16 面板、右欄≤4＋更多＋行商、主公軍府集中管理稱號／框且無跨頁捷徑、結算繁中戰報、關閉鈕 44px；2026-08-30 增加兩列指揮條、任務列語意 grid、單一滾動面板與入口唯一性契約；仍缺四尺寸截圖與實機證據關閉 VERIFY |
...
| WORK-019 | 依品質重置流程執行第一章四層驗收並限制完成宣告 | QA-003 | IN PROGRESS | 已完成自動化與角色審查修正；仍需真人玩家盲測、Android／WebView／TalkBack 證據與 P0／P1 關閉後才能結案 |
