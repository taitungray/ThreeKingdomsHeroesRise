...
- `已確認`：長文字有獨立欄位與省略邊界，切換命令面板會重設共用內容捲動位置；本次以 QA 暫時補入缺少的 runtime helper 後，僅完成代表畫面的幾何抽查。正式 `npm run test:ui` 會因缺少全域依賴而中止，不能列為功能通過。
- `不一致`：UI 已套用「暗金鐵木軍帳」材質語彙（`css/base.css`／`hud.css`／`panels.css`／`css/ui-redesign.css`／`css/ui-overhaul.css`），但 2026-08-31 實測 `npm test` 與正式 `npm run test:ui` 均未通過；最末壓縮樣式把必要文字降到 10–13px、Boss 按鈕壓到 30px 高，未符合 14px／44px 底線。
...
- `不一致`：頂欄主公牌具有開啟「主公軍府」的模板與事件，但目前會因 `titleUnlocked()` 未定義而中止，玩家路徑不可用。
- `不一致`：主公軍府模板含肖像／稱號／頭像框裝備、經驗條與進度摘要，但因上述 runtime 錯誤不能視為已完成功能。
...
- `已確認`：目前右側戰場常駐只顯示「更多」；軍務抽屜一次列出 11 個入口。多個入口目前失效或超出首章必要範圍，需依可用性閘門隱藏、鎖定或延後。
...
- `不一致`：裝備換裝模板存在；問天樓、演武場與日常副本模板也存在，但共同引用未定義的 `currentArmyPower()`，目前不可由正式玩家入口完成操作。
...
- `不一致`：戰鬥具名將怒氣大招特寫、暴擊與打擊浮字；主線目標指引 DOM 存在，但被 `css/ui-overhaul.css` 的 `display: none` 隱藏，不能列為目前可見功能。
- `已確認`：戰鬥全面接入實體 WAV 音效：無雙大招觸發專屬奧義音效、暴擊觸發加重打擊音效、Boss 破敵觸發震撼音效。
- `不一致`：圖鑑模板包含四大勢力名將收集共鳴與玉璧獎勵，但引用未定義的 `campaignClears()`，目前入口不可用；戰鬥喊話氣泡程式路徑存在。
