...
1. 本檔、`docs/production-rules.md`、`docs/ui-display-rules.md`、`docs/platform-and-release.md`。
2. `docs/current-game-spec.md` 與程式、資料、manifest 可直接驗證的現況。
3. `docs/qa-test-matrix.md`、`docs/visual-qa.md`、`docs/issues-and-prevention.md`。
4. 含 plan、roadmap、evaluation、completion 的文件只代表計畫或歷史，不是目前完成證明。
...
- 只有使用者要求畫面測試、截圖或視覺驗收時，才啟動瀏覽器流程，規則見 `docs/visual-qa.md`。
...
- 未通過 `docs/qa-test-matrix.md` 的硬門檻，不得在 roadmap 或 completion 文件標記「完成」。
...
實際可讀參考位於 `C:/IncenseAshes/`；若日後另有 `D:/Rayon/IncenseAshes/`，也只能作工程結構與規範比較。可採用流程、證據分級、QA 與文件治理方法；不得直接複製其題材、美術、Firebase、AdMob、生產簽名、商店素材、帳號或專案識別資料。整合紀錄見 `docs/reference-integration-audit.md`。
